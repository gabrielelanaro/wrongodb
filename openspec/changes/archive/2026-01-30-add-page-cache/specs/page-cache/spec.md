## Notes
These requirements describe a write-back cache that sits between `Pager` and `BlockFile`.
Key terms:
- **bounded eviction**: the cache has a fixed capacity; when full it must evict an unpinned page to admit a new one.
- **pinned**: a page that is in active use (e.g., by an iterator) and cannot be evicted.
- **dirty**: a cached page whose contents differ from what is on disk and must be written before eviction or checkpoint.
- **stable vs working**: “stable” pages are reachable from the last durable checkpoint; the first write must copy-on-write to a new “working” block id.
