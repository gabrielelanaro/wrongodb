...
fn insert_and_find_roundtrip() {
    ...
    let alice = coll
        ...
        .unwrap();
    let bob = coll
        .insert_one(&mut session, json!({"name": "bob", "age": 25}))
        .unwrap();
    ...
    let bobs = coll
        .find(&mut session, Some(json!({"name": "bob"})))
        .unwrap();
    assert_eq!(bobs.len(), 1);
    assert_eq!(bobs[0].get("age").unwrap().as_i64().unwrap(), 25);

    let missing = coll
        .find_one(&mut session, Some(json!({"name": "carol"})))
        .unwrap();
...
fn find_one_by_id_uses_main_table() {
    ...
    let alice_id = {
        ...
        let got = coll
            .find_one(&mut session, Some(json!({"_id": bob_id})))
            .unwrap()
            .unwrap();
    ...
    let mut session = db2.open_session();
    let got2 = coll2
        .find_one(&mut session, Some(json!({"_id": alice_id})))
        .unwrap()
        .unwrap();
...
fn document_crud_roundtrip() {
    ...
    let fetched = coll
        .find_one(&mut session, Some(json!({"_id": id.clone()})))
        .unwrap()
        .unwrap();
    ...
    coll.delete_one(&mut session, Some(json!({"_id": id.clone()})))
        .unwrap();
    let missing = coll
